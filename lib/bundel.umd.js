...
(function (global, factory) {
  ...
  */
  ...
  const eventQueue = sessionDB ? JSON.parse(sessionDB) : []; // 事件队列
  ...
  function _errorAjaxInit() {
    ...
    let _handleEvent = function (event, url) {
      try {
        if (event && event.currentTarget && event.currentTarget.status !== 200) {
          console.log("xhr错误", event); // 在此上报错误
        }
      } catch (e) {
    ...
    xmlhttp.prototype.send = function (url = ajaxUrl) {
      this.addEventListener("error", e => _handleEvent(e)); // 失败

      this.addEventListener("load", e => _handleEvent(e)); // 完成

      this.addEventListener("abort", e => _handleEvent(e)); // 取消
