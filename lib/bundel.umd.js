...
(function (global, factory) {
  ...
  */
  ...
  const __$timestampUser = t => {
    let status = false;
    const mark = getLocalSessionKey("URL_TIME"); // 比较
  ...
  */
  ...
  const _httpSend = opt => {
    console.log("pp", JSON.stringify(opt)); // 浏览器兼容sendBeacon方法
  };

  /* 缓存处理 */

  const sessionDB = sessionStorage.getItem("EVENT_QUEUE"); // export const pageHashMap = new Set([]); // 页面路由缓存
      ...
      if (g_config.error) {
        ...
        window.addEventListener("error", event => {
          ...
          if (target instanceof HTMLElement && ["LINK", "SCRIPT", "IMG"].indexOf(target.nodeName) !== -1) {
            errorModel("resource", Object.assign({}, {
  ...
  const pvModel = opt => {
    ...
    storageLocal("PAGE_CACHE", pageCache); // 如果是uv 就看是否在页面缓存中， yes 返回 no 添加缓存
  ...
  var _Pv = {
    init() {
      if (g_config.pv) {
        ...
        history.replaceState = interceptHistory("replaceState"); // 监听浏览器前进后退操作

        window.addEventListener("popstate", () => routeChange()); // 监听哈希路由变化

        window.addEventListener("hashchange", () => routeChange()); // 监听history跳转（replace模式）

        window.addEventListener("replaceState", () => routeChange()); // 监听history跳转（push模式）
